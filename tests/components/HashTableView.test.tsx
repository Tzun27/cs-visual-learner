...
describe("HashTableView", () => {
  ...
  it("renders one ellipse per live entry and prints each key as text", () => {
    const table = buildHashTable([1, 9, 17]); // all collide in bucket 1
  ...
  it("describes key count and bucket count in aria-label when no highlights present", () => {
    const table = buildHashTable([1, 2, 3]);
  ...
  it("draws a connector line for every non-empty bucket", () => {
    const table = buildHashTable([1, 9, 17]); // one non-empty bucket
    const { container } = render(<HashTableView table={table} />);
