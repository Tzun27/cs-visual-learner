...
describe("insertionSort", () => {
  it("returns the exact step sequence for [3, 1, 2]", () => {
    ...
    expect(steps).toEqual([
      { kind: "compare", indices: [0, 1], array: [3, 1, 2] },
      { kind: "swap", indices: [0, 1], array: [1, 3, 2] },
      { kind: "compare", indices: [1, 2], array: [1, 3, 2] },
      { kind: "swap", indices: [1, 2], array: [1, 2, 3] },
      { kind: "compare", indices: [0, 1], array: [1, 2, 3] },
      { kind: "done", array: [1, 2, 3] },
    ]);
  });

  it("yields only a 'done' step for an empty array", () => {
    expect([...insertionSort([])]).toEqual([{ kind: "done", array: [] }]);
  });

  it("yields only a 'done' step for a single-element array", () => {
    expect([...insertionSort([42])]).toEqual([{ kind: "done", array: [42] }]);
  });
