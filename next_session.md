...
55. **New viz wrappers compose `VizSection`, not a hand-rolled shell.** Hash-table, heap, pairing-heap, fibonacci-heap, and BST wrappers each shed ~50–75 lines of identical boilerplate — the `<section>` / responsive-grid / `CodePanel` / aria-live / `<dl>` / `<Controls>` shell lives in `VizSection.tsx` (renamed from `HashVizSection.tsx` once it absorbed the other families). A wrapper supplies: curated input + `highlightsFor`/`annotationFor` switches, `ariaLabel`/`codePanelAriaLabel` (exact strings — decision 25), a `renderView(currentStep)` closure (View prop shapes are incompatible across families, so a render-prop is used, not a string registry), and optionally `caption` (uppercase legend line — omit for ops like heapify) and `counters(visibleSteps)` (omit for ops like pairing-heap merge that have nothing to count). The `<dl>` grid columns derive from the counter array's length. These wrappers deliberately do NOT pass `onRunToCompletion` — preserve that. **Deferred** (need `topSlot` or `bottomSlot` extensions when next added): `BSTViz` (Insert-order toggle), `HeapInsertViz` (sequence toggle), `HeapExtractMinViz` (extracted-values strip), `TreeTraversalViz` (mode toggle), `HeapDecreaseKeyInteractive` (interactive form). Don't reintroduce the per-wrapper shell on simple cases.
...
Three threads. First, shipped two new lessons: **Bloom Filter** and **Fibonacci Heap** (six commits). Second, audited the Bloom + Fibonacci code under the `code-simplification` and `code-review-and-quality` skills, then applied four behaviour-preserving simplifications (one commit). Third, ran the heap/BST shell extraction — promoted `HashVizSection` to `VizSection` and migrated 10 heap/pairing/fibonacci/BST wrappers off their hand-rolled shells (six commits). Net effect at HEAD: 13 commits since prior session, all green: typecheck/lint/format clean, 826 unit tests at 100% line+branch+function+statement coverage on `src/lib/dataStructures/`, 135 Playwright e2e tests including 26 axe-core a11y routes, Chrome DevTools MCP verified across every changed page with zero console errors.

### Shell extraction: `HashVizSection` → `VizSection` (decision 55)

The `HashVizSection` was never hash-specific — it was just a step-driven viz shell that shipped during the hash-table family migration. It now serves every heap-family wrapper too (and BST search/delete). Two prop relaxations made the absorption possible: `caption` and `counters` are now optional (omit `caption` for ops with no parametric input to legend like heapify; omit `counters` for ops with nothing meaningful to count like pairing-heap merge).

10 wrappers migrated: `FibonacciHeapInsertViz`, `FibonacciHeapExtractMinViz`, `FibonacciHeapDecreaseKeyViz`, `PairingHeapMergeViz`, `PairingHeapDeleteMinViz`, `HeapifyViz`, `HeapDecreaseKeyViz`, `BSTSearchViz`, `BSTDeleteViz`. Each shed ~50 lines of hook-wiring + JSX. Net **−409 lines across 34 files** (10 migrations + 24 hash wrapper imports + the rename itself).

**Deferred** to a follow-up extension pass (`topSlot` / `bottomSlot` / interactive-form support): `BSTViz` (Insert-order toggle above the viz), `HeapInsertViz` (sequence toggle), `HeapExtractMinViz` (extracted-values strip between viz and annotation), `TreeTraversalViz` (4-mode toggle), `HeapDecreaseKeyInteractive` (index/value form).

### Bloom + Fibonacci audit findings (applied)

Four simplifications surfaced by the `code-simplification` audit:

1. `BloomFilterView` now imports and uses the exported `bloomFilterPopcount` helper instead of inlining the reduce.
2. `fibonacciHeap.linkChild`: collapsed a confused inline-debate comment ("…? Actually no -") into a clean one-sentence explanation.
3. `fibonacciHeapInsertSequence`: removed an unnecessary `wasMin` temp.
4. `fibonacciHeapDecreaseKeySequence`: collapsed two `if (parentId !== null)` checks into a single outer conditional.

The `code-review-and-quality` audit verdict was approve across all five axes (correctness, readability, architecture, security, performance). No follow-up findings.
